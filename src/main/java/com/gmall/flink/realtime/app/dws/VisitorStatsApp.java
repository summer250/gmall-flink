package com.gmall.flink.realtime.app.dws;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.gmall.flink.realtime.bean.VisitorStats;
 ...
 * -UniqueVisitApp从dwd_page_log层读取数据,计算独立访客,将结果发送到dwm_unique_visit
...
//        env.setStateBackend(new FsStateBackend("file:///Volumes/BigData/flink/gmall_flink-realtime/src/main/resources/checkpoint"));
        ...
        //2.3 从dwm_unique_visit主题中读取日志数据
        ...
        //2.4 从dwm_user_jump_detail主题中读取日志数据
...
//        userJumpJsonStrDS.print("userJump>>>>");
        ...
        //3.2 转换uv流
        SingleOutputStreamOperator<VisitorStats> uvStateDS = uvJsonStrDS.map(
                new MapFunction<String, VisitorStats>() {
                    @Override
                    public VisitorStats map(String jsonStr) throws Exception {
                        //将json格式字符串转换为json对象
        ...
        SingleOutputStreamOperator<VisitorStats> svStateDS = pvJsonStrDS.process(
        ...
        //3.4 转换跳出流
        SingleOutputStreamOperator<VisitorStats> userJumpStateDS = userJumpJsonStrDS.map(
                new MapFunction<String, VisitorStats>() {
                    @Override
                    public VisitorStats map(String jsonStr) throws Exception {
                        //将json格式字符串转换为json对象
        ...
        //TODO 4.将4条流合并到一起   注意:只能合并结构相通的流
        DataStream<VisitorStats> unionDS = pvStatsDS.union(uvStateDS, svStateDS, userJumpStateDS);

        //TODO 5.设置watermark以及提取事件时间
        SingleOutputStreamOperator<VisitorStats> visitorStateWithWatermarkDS = unionDS.assignTimestampsAndWatermarks(
                WatermarkStrategy
        ...
        //TODO 6.分组  按照地区,渠道,版本,新老访客维度进行分组,因为我们这里有4个维度,所以将他们封装为一个Tuple4
        KeyedStream<VisitorStats, Tuple4<String, String, String, String>> keyedDS = visitorStateWithWatermarkDS.keyBy(
                ...
                new ProcessWindowFunction<VisitorStats, VisitorStats, Tuple4<String, String, String, String>, TimeWindow>() {
                    ...
                    public void process(Tuple4<String, String, String, String> stringStringStringStringTuple4, Context context, Iterable<VisitorStats> elements, Collector<VisitorStats> out) throws Exception {
                        ...
                        for (VisitorStats visitorStats : elements) {
                            //获取窗口的开始时间
                            String startDate = sdf.format(context.window().getStart());
                            //获取窗口的结束时间
                            String endDate = sdf.format(context.window().getEnd());
